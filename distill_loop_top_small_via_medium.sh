#!/bin/bash
# Loop wrapper for distill_train_top_small_via_medium.sh (TAKD stage 3:
# medium->small). Mirrors distill_loop_top_T_sweep.sh's resubmit-on-timeout
# pattern.
#
# Usage (in a tmux or screen session):
#   bash distill_loop_top_small_via_medium.sh
...
echo "TAKD stage 3 (medium->small): SAVE_TAG=$SAVE_TAG"
...
while [ "$LOOP" -lt "$MAX_LOOPS" ]; do
    ...
    salloc \
        ...
        --gpus-per-node 4 \
        -A m3246 \
        bash "$SCRIPT_DIR/distill_train_top_small_via_medium.sh" \
        2>&1 | tee "$LOG_FILE"
    EXIT="${PIPESTATUS[0]}"
