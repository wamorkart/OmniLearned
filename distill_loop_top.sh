#!/bin/bash
# Continuous distillation training loop for the top-tagging student.
#
# Run inside a tmux session so the loop survives terminal disconnects:
#   tmux new-session -s distill_top 'bash /path/to/distill_loop_top.sh'
#
...
while [ "$LOOP" -lt "$MAX_LOOPS" ]; do
    ...
    salloc \
        ...
        --gpus-per-node 4 \
        -A m3246 \
        bash "$SCRIPT_DIR/distill_train_top.sh" \
        2>&1 | tee "$LOG_FILE"
    EXIT="${PIPESTATUS[0]}"
