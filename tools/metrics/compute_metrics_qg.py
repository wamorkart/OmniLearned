"""Compute AUC, accuracy, and background rejection at fixed signal efficiencies for qg.
...
Usage:
    python compute_metrics_qg.py --indir /path/to/eval/dir --tag <save_tag>
    python compute_metrics_qg.py --indir /path/to/eval/dir --tag <tag1> --tag <tag2> --tag <tag3>
        (multiple --tag also prints a mean +- std summary across reps)


/path/to/eval/dir = /pscratch/sd/m/mbenyas/OmniLearned/eval/qg_deepsets_ce, /pscratch/sd/m/mbenyas/OmniLearned/eval/qg_deepsets_a05
<save_tag> = train_qg_deepsets_small_ce_scratch, distill_qg_deepsets_small_scratch_a05_T4
...
def main():
    ...
    results = []
    for tag in args.tag:
        preds, labels = load_rank_files(args.indir, tag, "qg")
