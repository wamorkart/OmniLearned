...
# Wraps fine_tune_dctr_distill_pretrain_full500_reg52.sh with salloc auto-resubmit on timeout.
#
...
while [ "$LOOP" -lt "$MAX_LOOPS" ]; do
    ...
    salloc \
        ...
        --gpus-per-node 4 \
        -A m3246 \
        bash "$SCRIPT_DIR/fine_tune_dctr_distill_pretrain_full500_reg52.sh" \
        2>&1 | tee "$LOG_FILE"
    EXIT="${PIPESTATUS[0]}"
