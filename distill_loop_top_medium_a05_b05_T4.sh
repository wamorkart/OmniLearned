#!/bin/bash
# Loop wrapper for distill_train_top_medium_a05_b05_T4.sh (TAKD stage 1:
# large->medium, alpha=0.5 beta=0.5 T=4). Mirrors distill_loop_top_medium.sh's
# resubmit-on-timeout pattern.
#
# Usage (in a tmux or screen session):
#   bash distill_loop_top_medium_a05_b05_T4.sh
...
echo "TAKD stage 1 (large->medium, a05_b05): SAVE_TAG=$SAVE_TAG"
...
while [ "$LOOP" -lt "$MAX_LOOPS" ]; do
    ...
    salloc \
        ...
        --gpus-per-node 4 \
        -A m3246 \
        bash "$SCRIPT_DIR/distill_train_top_medium_a05_b05_T4.sh" \
        2>&1 | tee "$LOG_FILE"
    EXIT="${PIPESTATUS[0]}"
