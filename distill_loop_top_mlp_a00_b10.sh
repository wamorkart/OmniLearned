#!/bin/bash
# Continuous training loop for the pure-KD minimal-MLP top-tagging KD student
# (distill_top_mlp_small_scratch_a00_b10_T4), 4 nodes x 4 GPUs on the
# Perlmutter interactive queue. Mirrors distill_loop_top_mlp.sh.
#
# Run inside a screen session so the loop survives terminal disconnects:
#   screen -dmS distill_mlp_a00_b10 bash distill_loop_top_mlp_a00_b10.sh
#   screen -r distill_mlp_a00_b10   # to reattach

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
LOG_DIR=/pscratch/sd/t/twamorka/omnilearned/logs/distill_loop_top_mlp_a00_b10
mkdir -p "$LOG_DIR"
...
while [ "$LOOP" -lt "$MAX_LOOPS" ]; do
    ...
    salloc \
        ...
        --gpus-per-node 4 \
        -A m3246 \
        bash "$SCRIPT_DIR/distill_train_top_mlp_a00_b10.sh" \
        2>&1 | tee "$LOG_FILE"
    EXIT="${PIPESTATUS[0]}"
    set -e
