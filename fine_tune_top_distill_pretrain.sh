#!/bin/bash
# Fine-tune the KD-pretrained small student on top tagging (2-class).
# Starts from: best_model_distill_pretrain_s_scratch_a05_T4.pt
# Produces:    best_model_fine_tune_top_distill_pretrain_s_a05_T4.pt
#
# Run inside an salloc GPU interactive job, e.g.:
#   salloc -C gpu -q interactive -t 240 --nodes 1 --ntasks-per-node 4 \
#          --gpus-per-node 4 -A m3246 bash fine_tune_top_distill_pretrain.sh
...
cmd="omnilearned train \
  ...
  --pretrain-tag distill_pretrain_s_scratch_a05_T4 \
  --fine-tune \
