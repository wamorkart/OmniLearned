#!/bin/bash
# Distillation loop for a single no-interaction micro-student repetition run.
#
# Usage:
#   bash distill_loop_top_micro_noint_rep.sh distill_top_micro_noint_scratch_a05_T4_r1
#
# Same hyperparams as distill_loop_top_micro_rep.sh but the underlying training
# script drops --interaction/--local-interaction. Each salloc grabs 4 nodes x 4
# GPUs for 4 hours; resubmits on timeout, exits 0 when the 50-epoch run completes.

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
SAVE_TAG="${1:?Usage: $0 <save_tag>  e.g. distill_top_micro_noint_scratch_a05_T4_r1}"
...
while [ "$LOOP" -lt "$MAX_LOOPS" ]; do
    ...
    salloc \
        ...
        --gpus-per-node 4 \
        -A m3246 \
        bash "$SCRIPT_DIR/distill_train_top_micro_noint_rep.sh" \
        2>&1 | tee "$LOG_FILE"
    EXIT="${PIPESTATUS[0]}"
