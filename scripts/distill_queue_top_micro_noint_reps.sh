#!/bin/bash
# Queue manager for the 5 no-interaction micro-student replicate runs (r1-r5).
#
# Ablation: same KD setup as the micro reps (distill_queue_top_micro_reps.sh)
# but WITHOUT --interaction/--local-interaction, to check whether the micro KD
# result still holds when dropping the pairwise interaction matrix (fewer FLOPs).
#
# Keeps exactly 2 concurrent distill_loop_top_micro_noint_rep.sh jobs running at
...
#   screen -S micro_noint_reps bash distill_queue_top_micro_noint_reps.sh
#
# Each distill_loop_top_micro_noint_rep.sh internally resubmits on timeout and
...
QUEUE=(
    distill_top_micro_noint_scratch_a05_T4_r1
    distill_top_micro_noint_scratch_a05_T4_r2
    distill_top_micro_noint_scratch_a05_T4_r3
    distill_top_micro_noint_scratch_a05_T4_r4
    distill_top_micro_noint_scratch_a05_T4_r5
)
MAX_CONCURRENT=2
...
launch_next() {
    ...
    bash "$SCRIPT_DIR/distill_loop_top_micro_noint_rep.sh" "$TAG" \
...
log "=== All 5 reps complete. Queue manager exiting. ==="
