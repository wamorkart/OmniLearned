#!/bin/bash
# Continuous fine-tune training loop for JetClass CE-only baseline on
# Perlmutter interactive queue.
#
# Run inside a tmux session so the loop survives terminal disconnects:
#   tmux new-session -s fine_tune_jc 'bash fine_tune_loop_jetclass.sh'
#
# Each salloc grabs 1 node x 4 GPUs for 4 hours.
# When the allocation expires (non-zero exit), the loop resubmits.
# When training finishes all epochs (exit 0), the loop stops.
#
# Safety: MAX_LOOPS caps at 50 sessions (~200 node-hours).

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
LOG_DIR=/pscratch/sd/t/twamorka/omnilearned/logs/fine_tune_loop_jetclass
mkdir -p "$LOG_DIR"
...
while [ "$LOOP" -lt "$MAX_LOOPS" ]; do
    ...
    salloc \
        ...
        --gpus-per-node 4 \
        -A m3246 \
        bash "$SCRIPT_DIR/run_train.sh" ft_jetclass \
        2>&1 | tee "$LOG_FILE"
    EXIT="${PIPESTATUS[0]}"
