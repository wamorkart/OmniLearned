#!/bin/bash
# Quick status dashboard for the no-interaction micro-student replicate runs
# (r1-r5). No GPU needed. Run any time to see training/eval progress and
# queue-manager state:
#   bash distill_status_top_micro_noint_reps.sh
#
...
echo "=== No-interaction micro-student rep status [$(date '+%Y-%m-%d %H:%M:%S')] ==="
echo ""
...
TAGS = [
    ("distill_top_micro_noint_scratch_a05_T4_r1", "r1"),
    ("distill_top_micro_noint_scratch_a05_T4_r2", "r2"),
    ("distill_top_micro_noint_scratch_a05_T4_r3", "r3"),
    ("distill_top_micro_noint_scratch_a05_T4_r4", "r4"),
    ("distill_top_micro_noint_scratch_a05_T4_r5", "r5"),
]
...
print(f"  {'Label':<12}  {'Tag':<44}  Status")
print(f"  {'-'*12}  {'-'*44}  {'-'*26}")
...
echo ""
echo "Screen sessions (micro_noint_reps):"
screen -ls 2>/dev/null | grep "micro_noint_reps" || echo "  (none)"
