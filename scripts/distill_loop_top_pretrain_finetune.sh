...
# Wraps run_train.sh + configs/train/ft_top_pretrain.sh with salloc auto-resubmit on timeout.
#
...
LOG_DIR=/pscratch/sd/t/twamorka/omnilearned/logs/distill_loop_top_pretrain_finetune
mkdir -p "$LOG_DIR"
...
while [ "$LOOP" -lt "$MAX_LOOPS" ]; do
    ...
    salloc \
        ...
        --gpus-per-node 4 \
        -A m3246 \
        bash "$SCRIPT_DIR/run_train.sh" ft_top_pretrain \
        2>&1 | tee "$LOG_FILE"
    EXIT="${PIPESTATUS[0]}"
