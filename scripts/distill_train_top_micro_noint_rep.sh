...
# WITHOUT --interaction / --local-interaction (drops the pairwise interaction
# matrix -> fewer FLOPs). Ablation counterpart to distill_train_top_micro_rep.sh
# to check whether the micro KD result still holds without it.
#
...
#
# Usage (set tag before running, e.g. via distill_loop_top_micro_noint_rep.sh):
#   DISTILL_SAVE_TAG=distill_top_micro_noint_scratch_a05_T4_r1 bash distill_train_top_micro_noint_rep.sh
