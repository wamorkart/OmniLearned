...
# Wraps run_train.sh + configs/train/ft_top_pretrain_full500_reg52_a00_b10_v2.sh with salloc auto-resubmit on timeout.
#
...
while [ "$LOOP" -lt "$MAX_LOOPS" ]; do
    ...
    salloc \
        ...
        --gpus-per-node 4 \
        -A m3246 \
        bash "$SCRIPT_DIR/run_train.sh" ft_top_pretrain_full500_reg52_a00_b10_v2 \
        2>&1 | tee "$LOG_FILE"
    EXIT="${PIPESTATUS[0]}"
