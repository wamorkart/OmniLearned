#!/bin/bash
# Continuous training loop for the atlas_flav ftag fine-tune "v6" run
# (fine_tune_atlas_flav_m_localint_v6 = v5 recipe + --lr-factor 5, matching
# the OmniLearned paper's own fine-tuning recipe), 4 nodes x 4 GPUs = 16 GPUs
...
# train_loop_atlas_flav_ftag_m_localint_v5.sh, pointed at the v6 per-session
# script.
#
# Run inside a screen session so the loop survives terminal disconnects:
#   screen -dmS train_atlas_flav_ftag_m_localint_v6 bash scripts/train_loop_atlas_flav_ftag_m_localint_v6.sh
#   screen -r train_atlas_flav_ftag_m_localint_v6   # to reattach

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
LOG_DIR=/pscratch/sd/t/twamorka/omnilearned/logs/train_loop_atlas_flav_ftag_m_localint_v6
mkdir -p "$LOG_DIR"
...
while [ "$LOOP" -lt "$MAX_LOOPS" ]; do
    ...
    salloc \
        ...
        --gpus-per-node 4 \
        -A m3246 \
        bash "$SCRIPT_DIR/train_atlas_flav_ftag_m_localint_v6_interactive.sh" \
        2>&1 | tee "$LOG_FILE"
    EXIT="${PIPESTATUS[0]}"
    set -e
