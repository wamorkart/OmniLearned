#!/bin/bash
# Continuous distillation training loop for JetClass (pretrain_l teacher) on
# Perlmutter interactive queue.
#
# Run inside a tmux session so the loop survives terminal disconnects:
#   tmux new-session -s distill_jc_pl 'bash distill_loop_jetclass_pretrain_l.sh'
#
...
LOG_DIR=/pscratch/sd/t/twamorka/omnilearned/logs/distill_loop_jetclass_pretrain_l
mkdir -p "$LOG_DIR"
