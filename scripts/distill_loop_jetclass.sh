#!/bin/bash
# Continuous distillation training loop for JetClass on Perlmutter interactive queue.
#
# Run inside a tmux session so the loop survives terminal disconnects:
#   tmux new-session -s distill_jetclass 'bash /path/to/distill_loop_jetclass.sh'
#
...
LOG_DIR=/pscratch/sd/t/twamorka/omnilearned/logs/distill_loop_jetclass
mkdir -p "$LOG_DIR"
...
while [ "$LOOP" -lt "$MAX_LOOPS" ]; do
    ...
    salloc \
        ...
        --gpus-per-node 4 \
        -A m3246 \
        bash "$SCRIPT_DIR/distill_train_jetclass.sh" \
        2>&1 | tee "$LOG_FILE"
    EXIT="${PIPESTATUS[0]}"
