...
#   screen -dmS eval_atlas_flav_ftag_m_localint_v3 bash scripts/eval_loop_atlas_flav_ftag_m_localint_v3.sh
...
LOG_DIR=/pscratch/sd/t/twamorka/omnilearned/logs/eval_loop_atlas_flav_ftag_m_localint_v3
mkdir -p "$LOG_DIR"
...
while [ "$LOOP" -lt "$MAX_LOOPS" ]; do
    ...
    salloc \
        ...
        --gpus-per-node 4 \
        -A m3246 \
        bash "$SCRIPT_DIR/eval_atlas_flav_ftag_m_localint_v3_interactive.sh" \
        2>&1 | tee "$LOG_FILE"
    EXIT="${PIPESTATUS[0]}"
    set -e
