#!/bin/bash
# One-off driver: wait for the distillnet-GNN KD training to finish, then
# evaluate it on the test split and score it with compute_metrics_top.py
# (alongside the plain-distillnet baseline for comparison).
#
#   screen -dmS eval_gnn bash scripts/eval_after_gnn_distillnet.sh
#
...
BASELINE_TAG=distill_top_deepsets_distillnet_scratch_a05_T4
EVAL_CONFIG=top_deepsets_distillnet_gnn
...
LOOP_LOG_DIR=/pscratch/sd/t/twamorka/omnilearned/logs/distill_loop_top_deepsets_distillnet_gnn
SUMMARY="$LOOP_LOG_DIR/summary.log"
TRAIN_JSON="$CKPT_DIR/training_${TAG}.json"

OUT_LOG_DIR=/pscratch/sd/t/twamorka/omnilearned/logs/eval_after_gnn_distillnet
mkdir -p "$OUT_LOG_DIR"
...
{
    echo "===================================================================="
    echo "  distillnet-GNN KD eval  --  $(date '+%Y-%m-%d %H:%M:%S')"
    echo "  tag:      $TAG"
...
if ls "$EVAL_DIR"/outputs_"${BASELINE_TAG}"_top_test_rank*.npz >/dev/null 2>&1; then
    log "scoring plain-distillnet baseline for comparison"
