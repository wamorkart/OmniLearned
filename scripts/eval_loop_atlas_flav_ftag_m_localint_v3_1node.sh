...
# 1 node x 4 GPUs on the Perlmutter interactive queue. Copy of
...
#   screen -dmS eval_atlas_flav_ftag_m_localint_v3 bash scripts/eval_loop_atlas_flav_ftag_m_localint_v3_1node.sh
#   screen -r eval_atlas_flav_ftag_m_localint_v3   # to reattach
#
# One 1/6 test chunk (~4M jets) on 4 GPUs with bf16 finishes well inside one
# 240-min interactive session, so MAX_LOOPS=3 is only preemption headroom.

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
LOG_DIR=/pscratch/sd/t/twamorka/omnilearned/logs/eval_loop_atlas_flav_ftag_m_localint_v3_1node
mkdir -p "$LOG_DIR"
...
while [ "$LOOP" -lt "$MAX_LOOPS" ]; do
    ...
    salloc \
        ...
        --gpus-per-node 4 \
        -A m3246 \
        bash "$SCRIPT_DIR/eval_atlas_flav_ftag_m_localint_v3_1node_interactive.sh" \
        2>&1 | tee "$LOG_FILE"
    EXIT="${PIPESTATUS[0]}"
    set -e
