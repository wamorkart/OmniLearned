#!/bin/bash
# Save teacher logits from best_model_fine_tune_qg_pretrain_l.pt (large model fine-tuned
# on qg) for train and val splits, then build per-sample companion H5 files.
#
...
#          --gpus-per-node 4 -A m3246 bash save_teacher_logits_qg.sh
