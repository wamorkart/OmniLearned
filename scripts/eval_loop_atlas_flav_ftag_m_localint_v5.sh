#!/bin/bash
# Resubmit loop for the atlas_flav ftag localint _v5 TEST-split evaluation,
# 4 nodes x 4 GPUs on the Perlmutter interactive queue. Copy of
# eval_loop_atlas_flav_ftag_m_localint_v3.sh pointed at the _v5 per-session
# script.
#
# Run inside a screen session so the loop survives disconnects:
#   screen -dmS eval_atlas_flav_ftag_m_localint_v5 bash scripts/eval_loop_atlas_flav_ftag_m_localint_v5.sh
#   screen -r eval_atlas_flav_ftag_m_localint_v5   # to reattach
#
...
LOG_DIR=/pscratch/sd/t/twamorka/omnilearned/logs/eval_loop_atlas_flav_ftag_m_localint_v5
mkdir -p "$LOG_DIR"
...
while [ "$LOOP" -lt "$MAX_LOOPS" ]; do
    ...
    salloc \
        ...
        --gpus-per-node 4 \
        -A m3246 \
        bash "$SCRIPT_DIR/eval_atlas_flav_ftag_m_localint_v5_interactive.sh" \
        2>&1 | tee "$LOG_FILE"
    EXIT="${PIPESTATUS[0]}"
    set -e
