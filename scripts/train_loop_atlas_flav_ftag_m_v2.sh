#!/bin/bash
# Continuous training loop for the atlas_flav ftag fine-tune, attempt 2
# (fine_tune_atlas_flav_m_v2, no --local-interaction), 4 nodes x 4 GPUs on the
# Perlmutter interactive queue. Keeps resubmitting salloc jobs until training
# finishes (exit 0) or MAX_LOOPS is hit. Same pattern as
# train_loop_atlas_flav_ftag_m.sh, just pointed at the _v2 per-session script.
#
# Run inside a screen session so the loop survives terminal disconnects:
#   screen -dmS train_atlas_flav_ftag_m_v2 bash scripts/train_loop_atlas_flav_ftag_m_v2.sh
#   screen -r train_atlas_flav_ftag_m_v2   # to reattach

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
LOG_DIR=/pscratch/sd/t/twamorka/omnilearned/logs/train_loop_atlas_flav_ftag_m_v2
mkdir -p "$LOG_DIR"
...
while [ "$LOOP" -lt "$MAX_LOOPS" ]; do
    ...
    salloc \
        ...
        --gpus-per-node 4 \
        -A m3246 \
        bash "$SCRIPT_DIR/train_atlas_flav_ftag_m_v2.sh" \
        2>&1 | tee "$LOG_FILE"
    EXIT="${PIPESTATUS[0]}"
    set -e
